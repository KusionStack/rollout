...
const (
	...
	Rollout    ExpectedResourceType = "Rollout"
	Workflow   ExpectedResourceType = "Workflow"
	InPlaceSet ExpectedResourceType = "InPlaceSet"
)
...
func init() {
	ResourceInitializers = map[ExpectedResourceType]func() client.Object{
		...
		Rollout: func() client.Object {
			return &rolloutv1alpha1.Rollout{}
		},
		Workflow: func() client.Object {
			return &rolloutv1alpha1.Workflow{}
		},
	}
}
...
func (ae *ActiveExpectations) Expect(subject metav1.Object, kind ExpectedResourceType, name string, action ActiveExpectationAction) error {
...
func (ae *ActiveExpectations) ExpectUpdate(subject metav1.Object, kind ExpectedResourceType, name string, resourceVersion int64) error {
	if _, exist := ResourceInitializers[kind]; !exist {
		panic(fmt.Sprintf("kind %s is not supported for Active Expectation", kind))
	}
	...
	if err := expectation.(*ActiveExpectation).expectUpdate(kind, name, resourceVersion); err != nil {
		return fmt.Errorf("fail to expect %s/%s for action %d, %d: %s", kind, name, Update, resourceVersion, err)
	}
...
func (ae *ActiveExpectations) DeleteItem(subject metav1.Object, kind ExpectedResourceType, name string) error {
	if _, exist := ResourceInitializers[kind]; !exist {
		panic(fmt.Sprintf("kind %s is not supported for Active Expectation", kind))
	}
...
func (ae *ActiveExpectation) expect(kind ExpectedResourceType, name string, action ActiveExpectationAction) error {
	if action == Update {
		panic(fmt.Sprintf("update action is not supported by this method"))
	}
...
func (i *ActiveExpectationItem) isSatisfied(namespace string) (bool, error) {
	...
	case Update:
		...
		if err := i.Get(context.TODO(), types.NamespacedName{Namespace: namespace, Name: i.Name}, resource); err == nil {
			...
			if err != nil {
				// true for error
				return true, fmt.Errorf("fail to parse resource version %s of %s/%s to int64: %s", resource.(metav1.Object).GetResourceVersion(), i.Kind, i.Name, err)
			}
