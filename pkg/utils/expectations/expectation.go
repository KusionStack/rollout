...
func (r *ControllerExpectations) SatisfiedExpectations(controllerKey string) bool {
	if exp, exists, err := r.GetExpectations(controllerKey); exists {
		if exp.Fulfilled() {
			klog.V(4).Infof("Controller expectations fulfilled %s", controllerKey)
			return true
			...
			klog.V(4).Infof("Controller still waiting on expectations %#v", controllerKey)
			return false
		}
	} else if err != nil {
...
var ExpKeyFunc = func(obj interface{}) (string, error) {
	...
	if e, ok := obj.(*AccuracyExpectationItem); ok {
		return e.key, nil
	}
	if e, ok := obj.(*AccuracyResourceVersionExpectationItem); ok {
		return e.key, nil
	}
