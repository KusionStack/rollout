...
func (s *BackendRoutingControllerTestSuite) Test_ForBackends() {
	...
	s.Require().Eventually(func() bool {
		...
		s.Require().Contains(stableSVC.Labels, rolloutapi.LabelTemporaryResource)
		s.Require().Equal(rolloutapi.LabelValueTrafficLaneCanary, canarySVC.Spec.Selector[rolloutapi.LabelTrafficLane])
		s.Require().Contains(canarySVC.Labels, rolloutapi.LabelTemporaryResource)
...
func (s *BackendRoutingControllerTestSuite) Test_Route() {
	...
	s.Require().Eventually(func() bool {
		...
		s.Require().Contains(ingress.Labels, rolloutapi.LabelCanary)
		s.Require().Contains(ingress.Labels, rolloutapi.LabelTemporaryResource)
		return true
